                  ...
                  <article class="first mb-3">
                    ...
                    <div class="entry-meta">
                      @if ($firstFeature->user && $firstFeature->user->username)
                        <a href="{{ route('pages.author', $firstFeature->user->username) }}">{{ $firstFeature->user->name }}</a>
                      @else
                        <span>Unknown Author</span>
                      @endif
                      in
                      @if ($firstFeature->category && $firstFeature->category->slug)
                        <a href="{{ route('categories.show', $firstFeature->category->slug) }}">{{ $firstFeature->category->title }}</a>
                      @else
                        ...
                        <div class="entry-content col-8">
                          ...
                          <div class="entry-meta">
                            @if ($feature->user && $feature->user->username)
                              <a href="{{ route('pages.author', $feature->user->username) }}">{{ $feature->user->name }}</a>
                            @else
                              <span>Unknown Author</span>
                            @endif
                            in
                            @if ($feature->category && $feature->category->slug)
                              <a href="{{ route('categories.show', $feature->category->slug) }}">{{ $feature->category->title }}</a>
                            @else
                              <span>Uncategorized</span>
                            @endif
                            <br>
            ...
            <div class="pt-5 pb-5 pl-md-5 pr-5 align-self-center">
              ...
              <div class="entry-meta">
                ...
                @if ($editorsPick->category && $editorsPick->category->slug)
                  <a href="{{ route('categories.show', $editorsPick->category->slug) }}">{{ $editorsPick->category->title }}</a>
                @else
                  <span>Uncategorized</span>
                @endif
                <br>
                  ...
                  <div class="align-self-center">
                    ...
                    <div class="entry-meta align-items-center">
                      ...
                      @if ($article->category && $article->category->slug)
                        <a href="{{ route('categories.show', $article->category->slug) }}">{{ $article->category->title }}</a>
                      @else
                        <span>Uncategorized</span>
                      @endif
                      <br>
                    ...
                    <div class="post-content">
                      ...
                      <div class="entry-meta align-items-center">
                        ...
                        @if ($popular->category && $popular->category->slug)
                          <a href="{{ route('categories.show', $popular->category->slug) }}">{{ $popular->category->title }}</a>
                        @else
                          <span>Uncategorized</span>
                        @endif
                        <br>
